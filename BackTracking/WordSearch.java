...
class Solution{
    ...
    private static int[][] rc=new int[][]{{0,1},{0,-1},{1,0},{-1,0}};
    
    private static boolean helper(char[][] c,int index,String target,int row,int col){
        if(index==target.length()) return true;
        
        if(row<0 || row>=c.length || col<0 || col>=c[0].length || c[row][col]=='.') return false;
        
        if(c[row][col]==target.charAt(index)){
            char temp=c[row][col];
            c[row][col]='.';
            for(int i=0;i<4;i++){
                if(helper(c,index+1,target,row+rc[i][0],col+rc[i][1])) return true;
            }
            c[row][col]=temp;
        }
        return false;
    ...
    public static void main(String[] args){
        ...
        for(int i=0;i<m;i++){
            String temp=sc.next();
            for(int j=0;j<n;j++) c[i][j]=temp.charAt(j);
        }
        ...
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                if(helper(c,0,target,i,j)){
                    System.out.print(target+" can be traced");
                    return;
                }
            }
        }
        System.out.print(target+" cannot be traced");
    }
}