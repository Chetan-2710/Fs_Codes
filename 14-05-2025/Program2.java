...
class Program2{
    ...
    private static List<String> helper(String[] dict,String[] user){
        List<String> res=new ArrayList<>();
        Map<String,List<String>> map=new HashMap<>();
        
        for(String x:dict){
            String temp=x.toLowerCase();
            if(map.getOrDefault(temp,null)==null) map.put(temp,new ArrayList<>());
            map.get(temp).add(x);
        }
        ...
        for(String x:user){
            ...
            if(map.getOrDefault(lower,null)!=null){
                List<String> temp=map.get(lower);
                boolean flag=true;
                for(String y:temp){
                    if(x.equals(y)){
                        res.add(x);
                ...
                for(String y:dict){
                    if(y.length()==x.length()){
                        String temp1=x.toLowerCase(),temp2=y.toLowerCase();
                        boolean flag=true;
                        
                        for(int i=0;i<temp1.length();i++){
                            if(temp1.charAt(i)!=temp2.charAt(i)){
                                if(!vowels.contains(temp1.charAt(i)) || !vowels.contains(temp2.charAt(i))){
                                    flag=false;
                                    break;
                                }
                            }
    ...
    public static void main(String[] args){
        ...
        System.out.print(helper(dict,user));
