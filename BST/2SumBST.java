...
class Solution {
    ...
    private static boolean helper(TreeNode root1,TreeNode root2,int target){
        if(root1==null || root2==null) return false;
        
        int sum=root1.val+root2.val;
        if(sum==target) return true;
        if(sum<target){
            if(helper(root1.right,root2,target) || helper(root1,root2.right,target)) return true;
        }else{
            if(helper(root1.left,root2,target) || helper(root1,root2.left,target)) return true;
        }
