...
class MinAndMaxToaNumber{
    
    private static boolean find(TreeNode root,int val,List<Integer> temp){
        if(root==null)  return false;
        
        if(root.data<=val){
            temp.set(0,Math.max(temp.get(0),root.data));
        }
        if(root.data>=val){
            temp.set(1,Math.min(temp.get(1),root.data));
        }
        if(root.data==val) return true;
        
        if(val<root.data){
            if(find(root.left,val,temp)) return true;
        }
        if(find(root.right,val,temp)) return true;
        return false;
        
    }
    
    private static void place(TreeNode root,int val){
        if(val<root.data){
            if(root.left==null) root.left=new TreeNode(val);
            else place(root.left,val);
        }else{
            if(root.right==null) root.right=new TreeNode(val);
            else place(root.right,val);
        }
    }
    ...
    public static void main(String[] args){
        ...
        for(int i=1;i<nums.length;i++){
            place(root,Integer.parseInt(nums[i]));
        }
        
        String[] s=sc.nextLine().split(" ");
        List<List<Integer>> res=new ArrayList<>();
        for(int i=0;i<s.length;i++){
            // int[] temp=new int[]{Integer.MIN_VALUE,Integer.MAX_VALUE};
            
            List<Integer> temp=new ArrayList<>(Arrays.asList(Integer.MIN_VALUE,Integer.MAX_VALUE));
            find(root,Integer.parseInt(s[i]),temp);
            if(temp.get(0)==Integer.MIN_VALUE) temp.set(0,-1);
            if(temp.get(1)==Integer.MAX_VALUE) temp.set(1,-1);
            res.add(temp);
        }
